...
void PrintIndexVectorBoard(int row, int col){
	for(auto & elem : indexVectorBoard[row][col]){
		printf("%d ", elem);
	}
}
...
void SolveProblem(){
	...
	while(turns <= 1000){
		...
		for(int pieceIndex = 0; pieceIndex < piecesSize; pieceIndex++){
			...
			for(int i=0;i<4;i++){
				if(indexVectorBoard[currentRow][currentCol][i] == pieceIndex){
					currentHeight = i;
					break;
				}
			}
			...
			if(CheckIfBlueOrOutOfBound(nextRow, nextCol)){
				...
				// 흰색 칸으로 가는 경우,
				// printf("MOVEING TO WHTIE TILE!\n");
				// PrintIndexVectorBoard(currentRow, currentCol);
				// printf(" -> ");
				// PrintIndexVectorBoard(nextRow, nextCol);
				// printf("\nTo\n");

				int currentStackSize = indexVectorBoard[currentRow][currentCol].size();

				for(int i=currentHeight;i<currentStackSize;i++){
					// 현재 칸에 위치한 말부터 차례대로 다음 칸 위로 이동.
				...
				if(indexVectorBoard[nextRow][nextCol].size() >= 4)
					return;

				for(int i=currentHeight;i<currentStackSize;i++){
					indexVectorBoard[currentRow][currentCol].erase(indexVectorBoard[currentRow][currentCol].begin() + currentHeight);
				}
				// PrintIndexVectorBoard(currentRow, currentCol);
				// printf(" -> ");
				// PrintIndexVectorBoard(nextRow, nextCol);
				...
				for(int i=currentStackSize - 1;i>=currentHeight;i--){
